...
class Heap<E> {
	...
	public E remove() {
		if(array[1] == null) { // 만약 root가 비어있을 때 예외를 던지도록 함
	...
	private void siftDown(int idx, E target) {
		// comparator가 존재할 때 comparator를 인자로 넘겨준다.
		if(comparator != null) {
	...
	private void siftDownComparator(int idx, E target, Comparator<? super E> comp) {
		...
		int parent = idx; // 삭제노드로부터 시작할 부모를 가리키는 변수
		...
		while((child = getLeftChild(parent)) <= size) {
			...
			// 재배치할 노드가 자식 노드보다 작을 때 반복문 종료한다.
	...
	private void siftDownComparable(int idx, E target) {
		...
		Comparable<? super E> comp = (Comparable<? super E>) target; // 이해 안 됨.
